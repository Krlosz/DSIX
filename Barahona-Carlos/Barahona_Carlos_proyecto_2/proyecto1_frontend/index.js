(() => {
    const App = {
        ...
        htmlElements: {
            ...
            setSprites: document.querySelector("#sprites_id"),
            setEvos: document.querySelector("#evolucion_id"),
            setLocation: document.querySelector("#ubicaciones_id"),
        ...
        handlers: {
            handleFormSubmit: async (e) => {
                ...
                console.log("¿Do we already have the information for this pokemon?:" + data.isCached);
            },
        },
        templates: {
            searchButton: () => {
            ...
            pokemonCard: ({
                ...
                if (evolves_to.length > 0) {
                    ...
                    evolutionList += `<li>${species.name}${is_baby ? svgbaby : ''
                        }</li>`
                    do {
                        if (evolves_to.length > 1) {
                            for (i = 0; i < evolves_to.length; i++) {
                                evolutionList += `<li>${evolves_to[i].species.name}${evolves_to[i].is_baby ? svgbaby : ''
                                    }</li>`
                            }
                        } else {
                            evolutionList += `<li>${evolves_to[0].species.name}${evolves_to[0].is_baby ? svgbaby : ''
                                }</li>`
                        }
                ...
                let artwork = sprites.other;
                let off_artwork = artwork.home;
                let usable_3dArt = off_artwork.front_default;
                console.log(usable_3dArt)
                ...
                const abilitiesList = abilities.map(
                    ({ ability, is_hidden }) =>
                        `<li>${ability.name}${is_hidden ? svgEye : ''
                        }</li>`
                );

                htmlResponse = `<div class="card">
                <h1>${name} (${id})</h1>
                <p>Last Update: ${date_string}</p>
                <center><img src="${usable_3dArt}" width="250px height="250px""></center>
                <div class="info">`;
                ...
                if (App.htmlElements.setSprites.checked) {
                ...
                if (App.htmlElements.setLocation.checked) {
                    htmlResponse += `<div class="seccion">
                <h3>Catch Locations</h3>
               <p> ${encounters_data.map(a => a.location_area?.name.split("-").join(" ")).join("<br>")}</p>
                </div > `

                }

                if (App.htmlElements.setEvos.checked) {
                    htmlResponse += `<div class="seccion">
