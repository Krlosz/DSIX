((Utils) => {
  ...
  const hiddenIcon = `<svg width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  const App = {
      ...
      pokemonFinderFormOnSubmit: async (e) => {
        ...
        try {
          ...
          if (searchType == "pokemon") {
            ...
            const { chain } = await Utils.getEvolution(
              evolutions.evolution_chain.url
    ...
    templates: {
      ...
      errorCard: () => `<div id="infoBox><h1>There was an error</h1></div>`,
      pokemonCard: ({ id, name, weight, height, abilities, sprites, chain }) => {
        ...
        for (let i = 0; i < abilities.length; i++) {
          ...
          if (pleaseWork.is_hidden === true) {
            abilityArr.push(`<li>${pleaseWork.ability.name}${hiddenIcon}</li>`)
          }
        ...
        return `<div id="infoBox"><h4>${name}(${id})</h4>
            <section id="sprite_stats">
               <ul><h2>Sprites:<h2>${spriteFront}${spriteBack}</ul>
                <ul><h2>Weight/Height:</h2><p>${weight}/${height}</p>
                 </section>
                 <section id="Evo_ab">
                 <ul><h2>Evolution Chain</h2>${noSeparators}</ul>
                      <ul><h2>Abilities</h2>
                      ${noCommas}</ul>
                    </section></div>`;
      ...
      abilityCard: ({ name, pokemon }) => {
        ...
        const pokemonList = pokemon.map(
          ({ pokemon, is_hidden }) =>
            `<li><a target="_blank" href="${pokemon.url}">${pokemon.name}${is_hidden ? `${hiddenIcon}` : ""
            }</a></li>`
        );
        return `<div id="infoBox"><h1>${name}</h1><h2>Who Can Learn it?</h2><ul>${pokemonList.join("")}</ul></div>`;
      },
    },
  };
  App.init();
